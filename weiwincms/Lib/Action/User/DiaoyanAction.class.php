...
class DiaoyanAction extends UserAction{
	...
	public function suggestions(){
		...
		$check = $this->diaoyan_model->where(array('id'=>array('EQ',$tid)))->getField('token');
		if($check != $this->_get('token')){
			$this->error('非法操作');
		}
		$where['diaoyan_id'] = array('EQ',$tid);
		$where['jianyi'] = array('NEQ','');
		$count = M('diaoyan_user')->field('username,phone,jianyi')->where($where)->count();
		$Page = new Page($count,25);
		$show = $Page->show();
		$jianyi = M('diaoyan_user')->field('username,phone,jianyi')->where($where)->limit($Page->firstRow.','.$Page->listRows)->select();
		$this->assign('jianyi',$jianyi);
		$this->assign('page',$show);
