...
class KtvAction extends UserAction
{
        ...
        if (IS_POST) {
            if ($es_data == null) {
                if ($id = $data->add($_POST)) {
                    ...
                    M('Keyword')->add($data1);
                    $this->success('添加成功', U('Ktv/index', array('token' => session('token'))));
                    die;
                } else {
                ...
                if ($data->where($wh)->save($_POST)) {
                    ...
                    $this->success('修改成功', U('Ktv/index', array('token' => session('token'))));
                } else {
                    $this->error('操作失败');
                }
            }
    ...
    public function son_add()
    {
        ...
        if (IS_POST) {
            if ($check == null) {
                ...
                if ($t_son->add($_POST)) {
                    $this->success('添加成功', U('Ktv/son', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_son->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Ktv/son', array('token' => session('token'))));
                } else {
                    $this->error('操作失败');
                }
            }
    ...
    public function son_del()
    {
        ...
        if ($check == null) {
            ...
            if ($isok) {
                $this->success('删除成功', U('Ktv/son', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Ktv/son', array('token' => session('token'))));
            }
        }
    }
    ...
    public function housetype_add()
    {
        ...
        $id = (int) $this->_get('id');
        $token = $this->_get('token');
        $where = array('id' => $id, 'token' => $token,'thetype' => 'Ktv');
        $check = $t_housetype->where($where)->find();
        $son_data = M('Estate_son')->where(array('token' => session('token'),'thetype' => 'Ktv'))->field('id as sid,title')->select();
        $this->assign('son_data', $son_data);
        ...
        if (IS_POST) {
            if ($check == null) {
                ...
                if ($t_housetype->add($_POST)) {
                    $this->success('添加成功', U('Ktv/housetype', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_housetype->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Ktv/housetype', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('操作失败');
                }
    ...
    public function housetype_del()
    {
        ...
        if ($check == null) {
            ...
            if ($isok) {
                $this->success('删除成功', U('Ktv/housetype', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Ktv/housetype', array('token' => session('token'))));
            }
        }
    }
        ...
        if (IS_POST) {
            if ($check == NULL) {
                ...
                if ($t_album->add($_POST)) {
                    $this->success('添加成功', U('Ktv/album', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_album->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Ktv/album', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('操作失败');
                }
    ...
    public function album_del()
    {
        ...
        if ($check == null) {
            ...
            $isok = $housetype->where($where)->delete();
            if ($isok) {
                $this->success('删除成功', U('Ktv/album', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Ktv/album', array('token' => session('token'))));
            }
        }
    }
        ...
        if (IS_POST) {
            if ($check == null) {
                if ($t_impress->add($_POST)) {
                    $this->success('添加成功', U('Ktv/impress', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_impress->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Ktv/impress', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('操作失败');
                }
    ...
    public function impress_del()
    {
        ...
        if ($check == null) {
            ...
            if ($isok) {
                $this->success('删除成功', U('Ktv/impress', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Ktv/impress', array('token' => session('token'))));
            }
        }
    }
        ...
        if (IS_POST) {
            if ($check == null) {
                if ($t_expert->add($_POST)) {
                    $this->success('添加成功', U('Ktv/expert', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_expert->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Ktv/expert', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('操作失败');
                }
    ...
    public function expert_del()
    {
        ...
        if ($check == null) {
            ...
            if ($isok) {
                $this->success('删除成功', U('Ktv/expert', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Ktv/expert', array('token' => session('token'))));
            }
        }
    }
