...
class RepastAction extends UserAction{
	...
	public function out_execle(){
		$dish_order = M('Dish_order');
        $where = array('token' => $this -> _session('token'), 'cid' => $this -> _cid);
		$diningTable = M('Dining_table') -> where(array('cid' => $this -> _cid)) -> select();
		$orders = $dish_order->where($where)->order('time desc')->select();
		foreach($orders as $k=>$v){
			foreach($v as $key=>$value){
				switch ($key){
					case 'tableid':
						foreach($diningTable as $tk=>$tv){
							if($value == $tv['id']){
								$orders[$k][$key] = $tv['name'];
							}
						}
						break;
					case 'time':
						$orders[$k][$key] = date('Y-m-d H:i:s',$value);
						break;
					case 'reservetime':
						$orders[$k][$key] = date('Y-m-d H:i:s',$value);
						break;
					case 'takeaway':
						...
						break;
					case 'paid':
						$orders[$k][$key] = $value ? '已付款' : '未付款';
						break;
					case 'isuse':
						$orders[$k][$key] = $value ? '已就餐' : '未就餐';
						break;
				}
			}
		}
		$title = array(
			array('en'=>'name','cn'=>'姓名'),
			array('en'=>'tel','cn'=>'电话'),
			array('en'=>'total','cn'=>'数量'),
			array('en'=>'price','cn'=>'总价（元）'),
			array('en'=>'takeaway','cn'=>'就餐形式'),
			array('en'=>'tableid','cn'=>'餐台'),
			array('en'=>'paid','cn'=>'付款状态'),
			array('en'=>'isuse','cn'=>'就餐状态'),
			array('en'=>'time','cn'=>'订餐时间'),
			array('en'=>'reservetime','cn'=>'预约时间'),
		);
		out_execle('orders',$title,$orders);
	}
}
?>