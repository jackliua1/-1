        ...
        if (IS_POST) {
            ...
            if ($data->create() != false) {
                if ($id = $data->data($_POST)->add()) {
					...
					//妈B的，弄预约，这王八蛋
					$datar = M('Reservation');
					$wherer['token'] = session('token');
                    $wherer['thetype'] = $_REQUEST['thetype'];
					$rid=$data->order('id DESC')->find();
					
					//$datae = D('estate');
					$wheree['token'] = session('token');
                    $wheree['thetype'] = $_REQUEST['thetype'];
					$ride['res_id'] = $rid['id'];
					//$datae->where($wheree)->save($ride);
					M('estate')->where($wheree)->save($ride);
