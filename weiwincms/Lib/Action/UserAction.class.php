<?php class UserAction extends BaseAction{ 
	public $userGroup; 
	public $token; 
	public $user; 
	...
	protected function _initialize(){ 
		...
		$this->assign('userinfo',$userinfo); $this->userGroup=$userinfo; 
		$users=M('Users')->where(array('id'=>$_SESSION['uid']))->find(); 
		$this->user=$users; $this->assign('thisUser',$users); 
		$this->assign('viptime',$users['viptime']); if(session('uid')){ 
			if($users['viptime']<time()){ 
				session(null); session_destroy(); unset($_SESSION); 
				$this->error('您的帐号已经到期，请充值后再使用'); 
			} 
		} 
		$wecha=M('Wxuser')->where(array('token'=>session('token'),'uid'=>session('uid')))->find(); 
		$this->wxuser=$wecha; $this->assign('wxuser',$wecha); 
		$this->assign('wecha',$wecha); $this->token=session('token'); 
