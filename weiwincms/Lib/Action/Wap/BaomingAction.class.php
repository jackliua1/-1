...
class BaomingAction extends BaseAction {
	...
	public function add(){
		...
		if(empty($info))
		{
			D("BaomingOrder")->add($data);
		echo "<script>alert('报名成功');location.href='".$_SERVER["HTTP_REFERER"]."';</script>";}else
		{
	
		echo "<script>alert('您已经报过名了');location.href='".$_SERVER["HTTP_REFERER"]."';</script>";
