...
class DiningAction extends BaseAction{
	...
	public function cats(){
		$catWhere=array('parentid'=>0,'token'=>$this->token,'storeid'=>$_GET['storeid']);
		...
		if (isset($_GET['storeid'])){
			$where['storeid'] = intval($_GET['storeid']);
		}
	...
	public function dingdanpost(){
			...
			if (IS_POST){
				...
				$row['groupon']=0;
				$row['dining']=1;
				$row['setid']=intval($this->_get('setid'));
				...
				if($row['payment']==1){
				   ...
				   $cominfo=$this->company;
                                        ...
                                        $sys_email=$cominfo['email'];//公司邮箱
                                        $sys_phone=$cominfo['mp'];//店铺老板手机
                                        ...
                                        $content .="送餐查询:".$cominfo['tel']."";
                                        //店铺老板短信
                                        $st_body = strip_tags($this->sms_email_body());
                                        $sys_content ="微信店铺:".$this->company['name'].",".$st_body."";

                                        if ($user && $smsstatus == 1 && $content) {
                                                //给客户发送短信
                                                $smsrs = file_get_contents('http://api.smsbao.com/sms?u='.$user.'&p='.$pass.'&m='.$phone.'&c='.urlencode($content));
                                                //给店铺老板发送短信
                                                $smsrs_sys = file_get_contents('http://api.smsbao.com/sms?u='.$user.'&p='.$pass.'&m='.$sys_phone.'&c='.urlencode($sys_content));
                                        }
...
$mailbody = "订餐店铺：".$this->company['name']."<br>".$this->email_body()."<br>【温馨提示：您的订餐信息我们会尽快处理，欢迎您下次光临!!!】";//邮件内容 
...
$smtp->sendmail($smtpemailto, $smtpusermail, $mailsubject, $mailbody, $mailtype); 
					...
					$dining_company = M('dining_company')->where(array('token'=>$this->token))->find();
					if($dining_company){
						...
						if($is_open=='开启'){
							...
							$porduct_list = '';
							foreach ($id as $o){
							$products=M('dining')->where(array('id'=>$o))->find();//列表 table =  dining 
							$product_list .= chr(10).$products['name'].chr(32).chr(32).$info[$o]['count'].'份'.chr(32).chr(32).'单价：'.$info[$o]['price'].'元';
							}
							...
							chr(10).'订单类型：'.$type.
							chr(10).'订单状态：'.$pay.
							chr(10).'姓名：'.$order['truename'].
							chr(10).'电话：'.$order['tel'].
							chr(10).'地址：'.$order['address'].
							chr(10).'下单时间：'.date('Y-m-d H:i:s',$order['time']).
							chr(10).'配送时间:'.$order['buytime'].
							chr(10).'*******************************'.
							chr(10).$product_list.
							chr(10).'*******************************'.
							chr(10).'品种数量：'.$order['total'].
							chr(10).'合计：'.$order['price'].'元'.
							chr(10).'※※※※※※※※※※※※※※'.
							chr(10).'谢谢惠顾，欢迎下次光临'.
							chr(10).'订单编号：'.
							chr(10).$order['orderid'];
					
							$data['printed'] = 1;
							$result = M('product_cart')->where(array('id'=>$order['id']))->save($data);//更新字段
...
public function test_print(){
    	...
    	$mKey = $_POST['mkey'];//秘钥
    	$partner = $_POST['partner_id']; //合作id
    	$machine_code = $_POST['code'];//机器码
    	$msg = '
订单状态：未支付
姓名：小彬
电话：15982424080
地址：qiancongbin
下单时间：2014-06-05 14:28:34
配送时间:06月05日点
*******************************
...
谢谢惠顾，欢迎下次光临
订单编号';
		$result = $this->sendMsgToElink($msg,$apiKey,$mKey,$partner,$machine_code);
		echo '打印命令已发送，请检查';
}
	...
	//订单内容
	      public function email_body(){
                ...
                if ($orders){
                        ...
                        $str="订单类型：".$orderType."<br>订单编号：".$thisOrder['orderid']."<br>姓名：".$thisOrder['truename']."<br>电话：".$thisOrder['tel']."<br>地址：".$thisOrder['address']."<br>下单时间：".date('Y-m-d H:i:s',$thisOrder['time'])."<br>订单详情";
		  ...
		  public function sms_email_body(){
                $where['token']=$this->token;
                $where['wecha_id']=$this->wecha_id;
                $where['printed']=0;
                $this->product_model=M('dining');
                ...
                if ($orders){
                        ...
                        $str=$orderType.",订单:".$thisOrder['orderid'].",".$thisOrder['truename']."|".$thisOrder['tel']."|".$thisOrder['address'].",下单时间:".date('m-d H:i',$thisOrder['time']).",";
                        ...
                        if ($products){
                                ...
                                foreach ($products as $p){
                                        $products[$i]['count']=$carts[$p['id']]['count'];
                                        $str .=$p['name']."*".$p['price']."元*".$products[$i]['count']."份,";
                                        $i++;
                                }
                        }
                        $str .="合计:".$thisOrder['price']."元";
