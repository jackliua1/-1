...
class HotelsAction extends WapAction{
    ...
    public function hotel(){
        ...
        foreach ($sorts as $s){
            ...
            $hotels= M('Hotels_house')->where(array('sid'=>$s['id']))->select();
            $s['hotels']=$hotels;
            $list[] = $s;
        }
    ...
    public function order(){
		 $sorts = M('Hotels_house') -> where(array('cid' => $this -> _cid, 'token' => $this -> token)) -> select();
...
$content = "顾客{$_POST['name']}刚刚预定了{$sday}到{$eday}，{$days}天的{$sort['name']}，请您注意查看并处理";
if ($smsstatus == 1) {
    if ($content) {
...
$mailsubject = "您有新的订单，请注意查看并处理";//邮件主题 
