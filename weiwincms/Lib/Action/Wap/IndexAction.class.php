...
class IndexAction extends WapAction{
	...
	public function _initialize(){
		...
		//父类信息
		//添加了一个lpid的条件
		$allClasses=M('Classify')->where(array('token'=>$this->_get('token'),'status'=>1,'lpid'=>$_GET['lpid']))->order('sorts desc')->select();
	...
	public function index(){
		...
		//end
		//通过lpid和token查找需要显示的楼盘信息
		$where['lpid']=$_GET['lpid'];
		$lparr=D("lpinfo")->where($where)->find();
		//通过lpid和token查找需要显示的幻灯片
		$allflash=M('Flash')->where($where)->order('sort asc')->select();
		...
		$tpldata['color_id']=intval($tpldata['color_id']);
			//获取模板信息
			include('./weiwincms/Lib/ORG/index.Tpl.php');
			...
			foreach($info as $k=>$v){
				...
				if($info[$k]['url'] == ''){
						$info[$k]['url'] = U('Index/lists',array('classid'=>$v['id'],'token'=>$where['token'],'wecha_id'=>$this->wecha_id,'lpid'=>$_GET['lpid']));
					}
			}
			...
			if($tpldata['tpltypename'] == 'ktv_list' || $tpldata['tpltypename'] == 'yl_list'){
				...
				foreach($info as $key=>$val){
					...
					if($info[$key]['url'] == ''){
						$info[$key]['url'] = U('Index/lists',array('classid'=>$val['id'],'token'=>$where['token'],'wecha_id'=>$this->wecha_id,'lpid'=>$_GET['lpid']));
					}
	...
	public function lists(){
		...
		$classify = M('classify');
		//本分类信息		
		$info = $classify->where("id = $classid AND token = '$token'")->order('sorts asc')->find();	

		//是否有子类
		$sub = $classify->where("fid = $classid AND token = '$token'")->order('sorts asc')->select();
		...
		//有子类
			...
			//无子类 在模板中显示内容列表
				...
				$where['lpid']=$this->_get('lpid','intval');
				$db=D('Img');
	...
	public function content($contid='',$cid=''){
		...
		//获取模板
			include('./weiwincms/Lib/ORG/cont.Tpl.php');
