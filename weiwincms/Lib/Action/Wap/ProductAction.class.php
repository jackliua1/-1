...
class ProductAction extends BaseAction{
	...
	$products = $this->product_model->where($where)->order($order.' '.$method)->select();
		$this->assign('products',$products);
		$this->assign('metaTitle','商品');
		$this->assign('showMode', 2);//显示是一行1个，还是一行2个
		$this->display();
	}
	...
	public function orderCart(){
		...
		if (IS_POST){
			...
			if ($normal_rt||$groupon_rt||$dining_rt||$orderDining_rt){
				...
				if($printer_set['shop']==1)
				{
					$printercounts=$this->sms();
