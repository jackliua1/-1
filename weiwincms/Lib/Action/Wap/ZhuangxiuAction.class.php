...
class ZhuangxiuAction extends BaseAction
{
    ...
    {
		//广告方法
			$tpl=M('Wxuser')->where(array('token'=>$token))->find();
			$gid=M('Users')->field('gid')->find($tpl['uid']);
			$userGroup=M('User_group')->where(array('id'=>$gid['gid']))->find();
			$copyright=$userGroup['iscopyright'];
			$homeInfo=M('home')->where(array('token'=>$token))->find();
			$this->assign('iscopyright',$copyright);//是否允许自定义版权
    ...
    public function housetype()
    {
        ...
        $where       = array(
            'token' => $this->_get('token'),'thetype' => 'zhuangxiu'
        );
