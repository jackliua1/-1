...
class WapAction extends BaseAction{
    ...
    protected function _initialize(){
		...
		// var_dump($_COOKIE);
		if(isset($_COOKIE['wecha_id']) && !empty($_COOKIE['wecha_id'])){
		
			$_SESSION['wecha_id'] = $_COOKIE['wecha_id'];
		}
		//控制浏览器cssjs缓存版本号
		$this->assign('vtime',time());
		//var_dump($_SESSION);
        $this -> wecha_id = $this -> _get('wecha_id')?$this -> _get('wecha_id'):$_SESSION['wecha_id'];
        ...
        $catemenu = $menures;
        //自己重新定义catemenu查询结果    begin
        $catemenu=D("catemenu")->where(array('token' => $this -> token, 'status' => 1,'lpid'=>$_GET['lpid']))->select();
        //end
