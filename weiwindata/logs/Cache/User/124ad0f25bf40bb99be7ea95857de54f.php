...
<div class="cLineB">
  <h4>驻场经理信息维护</h4><a href="<?php echo U('Wechat_zhiye/index');?>" class="right btnGrayS vm">返回</a>
 </div> 
  <div class="msgWrap bgfc">
  <form action="" method="post" class="form-horizontal form-validate" novalidate="novalidate">
      <input type="hidden" name='ID' value="<?php echo ($list["ID"]); ?>">
  <div class="control-group">
    <label for="Name" class="control-label">驻场经理姓名：</label>
    <div class="controls">
      <input type="text" name="Name" id="Name" maxlength="10" class="span4" value="<?php echo ($list["Name"]); ?>" data-rule-required="true" readonly>
      <!-- <span class="maroon">*</span><span class="help-inline"></span> -->
    </div>
  </div>
  <div class="control-group">
    <label for="Tel" class="control-label">驻场经理电话：</label>
    <div class="controls">
      <input type="text" name="Tel" id="Tel" class="span4" data-rule-required="true" value="<?php echo ($list["Tel"]); ?>">
      <span class="maroon">*</span><span class="help-inline">请填写正确的手机号码</span>
    </div>
  </div>
  <div class="control-group">
		<label for="password" class="control-label">登录密码：</label>
		<div class="controls">
			<input type="password" name="password" id="password" class="span4" data-rule-required="true" value=""><span class="maroon">*</span><span class="help-inline">手机端登录使用,不修改则不填写</span>
		</div>
	</div>
  <div class="control-group">
    <label for="Uid" class="control-label">所属项目：</label>
    <div class="controls">
      <select name="Uid[]" class="chosen-select" data-placeholder="选择所属项目" id="Uid" multiple>
		<option value=""></option>
        <?php if(is_array($lp)): $i = 0; $__LIST__ = $lp;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$lp): $mod = ($i % 2 );++$i;?><option value="<?php echo ($lp["ID"]); ?>" <?php if(in_array($lp['ID'],$lps)): ?>selected<?php endif; ?>><?php echo ($lp["LouPanTitle"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
      </select>
      <!-- <input type="text" id="Uid" class="span4" data-rule-required="true" value="<?php echo ($list[0]["LouPanTitle"]); ?>" readonly> -->
      <!-- <span class="maroon">*</span><span class="help-inline"></span> -->
    </div>
  </div>
  <!-- <div class="control-group">
    <label for="addTime" class="control-label">创建日期：</label>
    <div class="controls">
      <input type="text" name="addTime" id="addTime" class="span4" data-rule-required="true" value="<?php echo (date('Y-m-d',$list["addTime"])); ?>" readonly>
     <!--  <span class="maroon">*</span><span class="help-inline"></span> -->
   <!--   </div>
  </div>
  <div class="control-group">
    <label for="BDTime" class="control-label">绑定日期：</label>
    <div class="controls">
      <?php if($list[0].BDTime == 0): ?><input type="text" name="BDTime" id="BDTime" class="span4" data-rule-required="true" value="" onClick="WdatePicker()">
        <span class="maroon">*</span><span class="help-inline"></span>
      <?php else: ?>
        <input type="text" name="BDTime" id="BDTime" class="span4" data-rule-required="true" value="<?php echo (date('Y-m-d',$list["BDTime"])); ?>">
        <span class="maroon">*</span><span class="help-inline"></span><?php endif; ?>
    </div>
  </div> -->
   <div class="form-actions">
      <button id="bsubmit" type="submit" data-loading-text="提交中..." class="btnGreen">保存</button>　<a href="<?php echo U('Wechat_zhiye/index');?>" class="btnGray vm">取消</a>
    </div>
</form>
  </div> 
 <script>
	$('.chosen-select').chosen({search_contains: true,width:"250px",allow_single_deselect : true});
 </script>
