...
<div class="cLineB">
  <h4>修改片区经理</h4><a href="<?php echo U('Districtmanager/index',array('token'=>$token));?>" class="right btnGrayS vm" >返回</a>
 </div> 
  <div class="msgWrap bgfc">
  <form action="<?php echo U('Districtmanager/updates',array('token'=>$token));?>" method="post" class="form-horizontal form-validate" novalidate="novalidate">
	<div class="control-group">
		<label for="title" class="control-label">片区经理姓名：</label>
		<div class="controls">
			<input type="text" name="areaName" id="title" maxlength="10" class="span4" value="<?php echo ($lists["areaName"]); ?>" data-rule-required="true">
		</div>
	</div>
	<div class="control-group">
		<label for="Tel" class="control-label">片区经理电话：</label>
		<div class="controls">
			<input type="text" name="areaTel" id="Tel" class="span4" data-rule-required="true" value="<?php echo ($lists["areaTel"]); ?>"><span class="maroon">*</span><span class="help-inline">用户登录时使用</span>
		</div>
	</div>
	<div class="control-group">
		<label for="Tel" class="control-label">登录密码：</label>
		<div class="controls">
			<input type="password" name="password" id="password" class="span4" data-rule-required="true" value=""><span class="maroon">*</span><span class="help-inline">用户登录时使用的密码</span>
		</div>
	</div>
	  <input type="hidden" name="Token" id="Teles" class="span4" data-rule-required="true" value="<?php echo ($lists["Token"]); ?>"><span class="maroon"></span><span class="help-inline"></span>
	  <input type="hidden" name="ID" id="Tels" class="span4" data-rule-required="true" value="<?php echo ($lists["ID"]); ?>"><span class="maroon"></span><span class="help-inline"></span>


	  <!--<div class="control-group">-->
		  <!--<label for="usersname" class="control-label">经纪公司：</label>-->
		  <!--<div class="controls">-->

			  <!--<select name="usersname" style="height:30px;">-->
				  <!--<?php if(is_array($list)): foreach($list as $key=>$vo): ?>-->
				  <!--<option   value="<?php echo ($vo["id"]); ?>" <?php if($vo[id] == $lists[brokerage_id]): ?>selected<?php endif; ?>><?php echo ($vo["usersname"]); ?></option>-->
