...
<div class="content">
		  ...
		  <h4>经纪人管理</h4>
			<div class="searchbar right">  
				<form method="post" action="<?php echo U('Share/index',array('token'=>$token));?>">
				  ...
				  <select name="brokerage_id" data-placeholder="选择所属公司" id="brokerage_id" style="" class="chosen-select">
					...
					<?php if(is_array($type)): $i = 0; $__LIST__ = $type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['id']); ?>" <?php if($vo[id] == $brokerage_id): ?>selected<?php endif; ?>><?php echo ($vo['usersname']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
...
<li <?php if($tab == 'set'): ?>class="selectTag"<?php endif; ?>>
          ...
          <div class="cLine">
            <div class="pageNavigator left">
                <a href="<?php echo U('Share/addAgent',array('token'=>$token));?>" class="btnGrayS vm bigbtn">
                    <img src="tpl/User/default/common/images/text.png" class="vm">添加经纪人
                </a>
                &nbsp;
               <span href="<?php echo U('Share/excinagent',array('token'=>$token));?>" class="btnGrayS vm bigbtn" id="import">
                    <img src="tpl/User/default/common/images/text.png" class="vm">导入经纪人
                </span>
                &nbsp;
                <a href="<?php echo U('Share/agentoutexc',array('token'=>$token,'Tel'=>$Tel,'brokerage_id'=>$brokerage_id));?>" class="btnGrayS vm bigbtn">
             ...
             <script type="text/javascript">
                ...
                hide();
                window.location="<?php echo U('Share/index');?>";
                });
                  //点击取消
              ...
              $("#dvMsk").remove();
               ...
               <div id="showDiv" style="display:none;">
                 ...
                 <div class="imtem" style="width:300px;">
                  ...
                  <form action="<?php echo U('Share/excinagent',array('token'=>$token));?>" method="post" enctype="multipart/form-data">
          ...
          <tbody>
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
              ...
              <td>
                <?php if($list['usersname'] == null): ?>未选择<?php else: echo ($list['usersname']); endif; ?>
              </td>
              <td><?php echo (date("Y-m-d H:i:s",$list['Mtime'])); ?></td>
              <td>
                <a href="<?php echo U('Share/modifyAgent',array('id'=>$list['ID'],'token'=>$list['Token']));?>" class="btn">编辑</a>
                &nbsp;<a href="<?php echo U('Share/del',array('id'=>$list['ID']));?>" onclick="return(confirm('确定要删除吗？'))" class="btn">删除</a></a>
              </td>
