...
<div class="cLineB">
  <h4>修改邀请码</h4><a href="<?php echo U('Wechat_behavior/invitationcode');?>" class="right btnGrayS vm" style="margin-top:-27px">返回</a>
 </div> 
  <div class="msgWrap bgfc">
  <form action="" method="post" class="form-horizontal form-validate" novalidate="novalidate">
  	<div class="control-group">
  		<input type="hidden" name="id" value="<?php echo ($list['id']); ?>">
		<label for="title" class="control-label" style="width:160px;">适用项目：</label>
			<select name="lpid" style="height:30px;">
				<?php if(is_array($lpinfo)): $i = 0; $__LIST__ = $lpinfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['id']); ?>" <?php if($vo['id'] == $list['lpid']): ?>selected<?php endif; ?>><?php echo ($vo['LouPanTitle']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
			</select>
	</div>
	<div class="control-group">
		<label for="title" class="control-label" style="width:160px;">适用类型：</label>
			<label for="zy"><input type="radio" value="0" name="type" id="zy" <?php if($list['type'] == 0): ?>checked<?php endif; ?>>置业顾问</label>&nbsp;&nbsp;
			<label for="ac"><input type="radio" value="1" name="type" id="ac" <?php if($list['type'] == 1): ?>checked<?php endif; ?>>案场经理　</label>&nbsp;&nbsp;
	</div>
	<div class="control-group">
		<label for="SrTime" class="control-label">邀请码：</label>
		<div class="controls">
			<input type="text" name="code" id="code" class="span4" value="<?php echo ($list['code']); ?>" data-rule-required="true">
			<span class="maroon">*</span><span class="help-inline">请输入6位数的邀请码，字母或数字</span>
		</div>
	</div>
	<div class="control-group">
		<label for="SrTime" class="control-label">状态：</label>
		<div class="controls">
			<label for="no"><input type="radio" value="0" name="status" id="no" <?php if($list['status'] == 0): ?>checked<?php endif; ?>>无效</label>&nbsp;&nbsp;
			<label for="yes"><input type="radio" value="1" name="status" id="yes" <?php if($list['status'] == 1): ?>checked<?php endif; ?>>有效</label>&nbsp;&nbsp;
		</div>
	</div>
	<div class="control-group">
		<label for="SrTime" class="control-label">有效时间：</label>
		<div class="controls">
			<input name="endtime" id="intro" class="px" value="<?php echo (date('Y-m-d',$list['endtime'])); ?>" rows="5" value="" onClick="WdatePicker()"/>
		</div>
	</div>
	<div class="form-actions">
		<button id="bsubmit" type="submit" data-loading-text="提交中..." class="btnGreen" onclick="return abc();">修改</button>
	</div>
