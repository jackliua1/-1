...
<div class="content">
          <div class="cLineB"><h4>经纪公司</h4></div>
...
<li <?php if($tab == 'set'): ?>class="selectTag"<?php endif; ?>>
          ...
          <div class="cLine">
            <div class="pageNavigator left">
                <a href="<?php echo U('Brokerage/add',array('token'=>$token));?>" class="btnGrayS vm bigbtn">
                    <img src="tpl/User/default/common/images/text.png" class="vm">添加经纪公司
                </a>
                &nbsp;
				<span href="<?php echo U('Brokerage/excin',array('token'=>$token));?>" class="btnGrayS vm bigbtn" id="import" style="width:auto;">
                    <img src="tpl/User/default/common/images/text.png" class="vm">导入经纪公司
                </span>
                &nbsp;
                <a href="<?php echo U('Brokerage/excout',array('token'=>$token));?>" class="btnGrayS vm bigbtn">
                    <img src="/tpl/User/default/common/images/text.png" class="vm">导出经纪公司
                </a>
            </div>
			<!--导入-->
             <script type="text/javascript">
                ...
                hide();
                window.location="<?php echo U('Brokerage/index');?>";
                });
                  //点击取消
              ...
              $("#dvMsk").remove();
               ...
               <div id="showDiv" style="display:none;">
                 ...
                 <div class="imtem" style="width:300px;">
                  <a href="tpl/User/default/common/tmp/经纪公司.xls"><button class="btuFile"  id="" style="margin-top:5px;margin-left:10px;padding:5px;">下载模板</button></a>
                  </div>
                  <form action="<?php echo U('Brokerage/excin',array('token'=>$token));?>" method="post" enctype="multipart/form-data">
            ...
            <input name="delall" type="hidden" value="del">
             <input name="wxid" type="hidden" value="gh_423dwjkewad">
              <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
                <thead>
               <tr>                        
          <th class="span3">经纪公司名称</th>
          <th class="span1">联系人名称</th>
          <th class="span3">经纪公司电话</th>
          <th class="span1">经纪公司地址</th>
		  <th class="span1">所在区域</th>
          <th class="span1">片区经理</th>
          <th class="span1">操作</th>
        </tr>
          </thead>
          <tbody>
            <?php if($list == ''): ?><tr>
              <td colspan="6" style="text-align:center; height:30px;"><strong>暂无经纪公司信息.</strong></td>
              </tr>
            <?php else: ?>
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
              <td><span class="label"> <?php echo ($list['usersname']); ?></span></td>
              <td><?php echo ($list['contacts']); ?></td>
              <td><?php echo ($list['tel']); ?></td>
              <td><?php echo ($list['address']); ?></td>
			  <td><?php echo ($list['in_area']); ?></td>
                  <td><?php echo ($list['areaName']); ?></td>
              <td>
                <a href="<?php echo U('Brokerage/add',array('id'=>$list['id'],'token'=>$token));?>" class="btn">编辑</a>
                &nbsp;<a href="<?php echo U('Brokerage/del',array('id'=>$list['id'],'token'=>$token));?>" onclick="return(confirm('确定要删除吗？'))" class="btn">删除</a></a>
              </td>
		  ...
		  <script>
			$(".gopage span").on('click',function(){
				var page = $(".page_num").val();
				var re = /^[1-9][0-9]*$/;
				if(!re.test(page)){
					alert("请输入正确的页码");
				}
				location.href = "<?php echo U('Brokerage/index',array('token'=>$token));?>&p="+page;
			});
		  </script>
        </div>