...
<div class="content">
  ...
  <div class="cLineD"></div>
      ...
      <div class="cLine">
        <div class="ftip" style="margin:0 auto">项目奖励类型的设置</div>
      </div>

	<div class="msgWrap form">
		<ul id="tags" style="width:100%">
			 <li>
          <a href="<?php echo U('Wechat_behavior/rule',array('token'=>$token));?>">活动细则</a> 
        </li>
        <li>
          <a href="<?php echo U('Wechat_behavior/salesstatus',array('token'=>$token));?>">项目销售状态</a> 
        </li>
        <li class="selectTag">
          <a href="<?php echo U('Wechat_behavior/rewardparam',array('token'=>$token));?>">奖励类型</a> 
        </li>
        <li>
          <a href="<?php echo U('Wechat_behavior/rewardinfo',array('token'=>$token));?>">项目奖励</a> 
        </li>
        <li>
          <a href="<?php echo U('Wechat_behavior/agenttype',array('token'=>$token));?>">会员类别</a> 
        </li>
        <li>
          <a href="<?php echo U('Wechat_behavior/invitationcode',array('token'=>$token));?>">邀请码</a> 
        </li>
        <li>
          <a href="<?php echo U('Wechat_behavior/shareindex',array('token'=>$_GET['token']));?>">首页分享</a> 
        </li>
  ...
  <div class="cLine">
    <div class="pageNavigator left">
       <a href="<?php echo U('Wechat_behavior/rewardparamadd',array('token'=>$token));?>" class="btnGrayS vm bigbtn">
            <img src="tpl/User/default/common/images/text.png" class="vm">新增类型
        </a>
    </div>
    <div class="clr"></div>
  </div>
  ...
  <div class="msgWrap">
    <form method="post" action="" id="info">
      ...
      <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
        <thead>
          <tr>
            <th class="span3">奖品名称</th>
            <th class="span3">奖励类型</th>
            <th class="span3">奖励金额/百分比</th>
            <th class="span3">参数说明</th>
            <th class="span3">操作</th>
          </tr>
        </thead>
        <tbody>
          <?php if($list == ''): ?><tr>
            <td colspan="6" style="text-align:center; height:30px;"><strong>暂无奖励类型设置</strong></td>
            </tr>
          <?php else: ?>
          <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
            ...
            <td>
              <span class="label">
                <?php if($list['rewardparam'] == 0): ?>佣金百分比<?php else: ?>实物金额<?php endif; ?>
              </span>
            </td>
            <td><span class="label"> <?php echo ($list['rewardamount']); ?></span></td>
            <td><span class="label"> <?php echo ($list['paraminstr']); ?></span></td>
            
            <td>
              <a href="<?php echo U('Wechat_behavior/rewardparamOfModule',array('id'=>$list['id'],'token'=>$list['token']));?>" class="btn">编辑</a>
              &nbsp;<a href="<?php echo U('Wechat_behavior/rewardparamOfDel',array('id'=>$list['id'],'token'=>$list['token']));?>" onclick="return(confirm('确定要删除吗？'))" class="btn">删除</a></a>
            </td>
