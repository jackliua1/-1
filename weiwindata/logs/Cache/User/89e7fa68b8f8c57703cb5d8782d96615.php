   ...
   微信接口不支持删除分组操作，所以添加的时候请您谨慎操作，如需删除请到公众平台删除，本系统会与公众平台同步
          ...
          <div class="cLine">
            <div class="pageNavigator left">
      				<a href="<?php echo U('Share_activite/add');?>" class="btnGrayS vm bigbtn">
      					<img src="tpl/User/default/common/images/text.png" class="vm">新增活动
      				</a>
      				&nbsp;
      				&nbsp;
      				<!-- <a href="<?php echo U('User/Wechat_group/provinceset');?>" class="btnGrayS vm bigbtn">
      					<img src="tpl/User/default/common/images/text.png" class="vm">维护省份
      				</a> -->
            </div>
            <div class="clr"></div>
          </div>
          ...
          <input name="delall" type="hidden" value="del">
           <input name="wxid" type="hidden" value="gh_423dwjkewad">
            <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
              ...
              <tbody>
              <?php if($list == null): ?><tr>
                  <td colspan="9" style="text-align:center; height:30px;"><strong>暂无分享活动</strong></td>
                </tr><?php endif; ?>
    			   <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                  <td width="100px"><?php echo ($list["sharename"]); ?></td>
                  <td width="100px">
                    <?php if(is_array($lparr)): $i = 0; $__LIST__ = $lparr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo['id'] == $list['loupanid']): echo ($vo["LouPanTitle"]); endif; endforeach; endif; else: echo "" ;endif; ?>
                  </td>
                  <td><div class="cateimg"><img src="<?php echo ($list["shareloge"]); ?>" class="cateimg_small"  /><img src="<?php echo ($list["shareloge"]); ?>" class="cateimg_big"  /></div></td>
                  <td><?php echo (date("Y-m-d",$list["starttime"])); ?></td>
                  <td><?php echo (date("Y-m-d",$list["endtime"])); ?></td>
				          <td><?php echo ($list["sharereward"]); ?></td>
                  <td><?php echo ($list["rewardnum"]); ?></td>
                  <td>
                    <?php if(($list['starttime'] <= $time) && ($list['endtime'] >= $endtime)): ?><span style="color:green">进行中</span>
                      <?php elseif($list['starttime'] > $time): ?>
                        <span style="color:red">未开始</span>
                      <?php else: ?>
                        <span style="color:red">已结束</span><?php endif; ?>
                  </td>
                  <td class="norightborder">
    						<a href="<?php echo U('Share_activite/edit',array('id'=>$list['id'],'token'=>$list['token']));?>">修改</a>&nbsp;&nbsp;
                <a href="<?php echo U('Share_activite/del',array('id'=>$list['id'],'token'=>$list['token']));?>"  onclick="return(confirm('确定要删除吗？'))">删除</a>
    				   </td>
