          ...
          <div class="cLineB"><h4>领取佣金操作</h4></div>
			...
			<div class="msgWrap form">
				<ul id="tags" style="width:100%">
					<li>
            <a href="?g=User&m=Share&a=index&token=<?php echo ($token); ?>">会员</a> 
          </li>
          <li class="selectTag">
            <a href="?g=User&m=Share&a=reward&token=<?php echo ($token); ?>">佣金信息</a> 
          </li>
					<!-- <li>
            <a href="?g=User&m=Share&a=credit&token=<?php echo ($token); ?>">积分信息</a> 
          </li>	 -->
         ...
         <div style="text-align:left; text-indent:30px; line-height:40px;"> 
                           <!--  <div style="float:left;">
                            ...
                            <form action="__URL__/reward" method='get'>
                              <span>
                                会员：
                                <input type="text" name="jjrname" placeholder="请输入会员姓名" value="<?php echo ($jjrname); ?>" style="border:1px solid #999; padding-left:5px; border-color:#fff #fff #ccc #fff; outline:none; height:20px;">
                              </span>
                              <span>
                                佣金状态：
                                <select name="rewardstatus" style="border-color:#fff #fff #ccc #fff; outline:none; background:none;  outline:none; " >
                                  <option value="" <?php if($rewardstatus == ''): ?>selected<?php endif; ?>>--请选择--</option>
                                  <option value="0" <?php if($rewardstatus == '0'): ?>selected<?php endif; ?>>获取佣金</option>
                                  <option value="1" <?php if($rewardstatus == '1'): ?>selected<?php endif; ?>>佣金取现</option>
                                  <option value="2" <?php if($rewardstatus == '2'): ?>selected<?php endif; ?>>取现申请</option>
                                  <option value="-1" <?php if($rewardstatus == '-1'): ?>selected<?php endif; ?>>购买商品</option>
                                  <option value="-99" <?php if($rewardstatus == '-99'): ?>selected<?php endif; ?>>佣金撤销</option>
                                </select>
                              </span>
            ...
            <table id="sbc" class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
             ...
             <!-- <th class="span3">客户姓名</th>
              ...
              <!-- <td style="text-align:center;">
                <?php if($list['rewardstatus'] == 0): ?><input type="checkbox" name="reward" value="<?php echo ($list['id']); ?>" id="oneinfo">
                <?php else: ?>
                  <input type="checkbox" disabled="true"><?php endif; ?>
              </td> -->
              <label for="oneinfo">
            <!-- <td><?php echo ($list['customname']); ?></td>
            <td><?php echo ($list['loupanname']); ?></td> -->
            <td><?php echo ($list['Name']); ?></td>
            <!-- <td><?php echo ($list['statusname']); ?></td>
            <td><?php echo ($list['rewardname']); ?></td> -->
            <td>
              <?php if(($list['rewardstatus'] == 0) || ($list['rewardstatus'] == '-99')): ?>+<?php else: ?>-<?php endif; echo ($list['rewardamount']); ?>
            </td>
            <td><?php echo (date('Y-m-d',$list['srtime'])); ?></td>
            </label>
            <td align="center">
              <?php if($list['rewardstatus'] == 0): ?><!-- <span class="label">
                  <a href="<?php echo U('Share/lqone',array('token'=>$list['token'],'id'=>$list['id']));?>" class="lingqu">领取</a>
                </span> -->
                <span class="label" style="color:#00f">获取佣金</span>
              <?php elseif($list['rewardstatus'] == 1): ?>
                <span class="label" style="color:#f00">佣金取现</span>
              <?php elseif($list['rewardstatus'] == 2): ?>
                <span class="label" style="color:#0f0">取现申请</span>
              <?php elseif($list['rewardstatus'] == '-1'): ?>
                <span class="label" style="color:#0f0">购买商品</span>
              <?php elseif($list['rewardstatus'] == '-99'): ?>
                <span class="label" style="color:#333">佣金撤销</span><?php endif; ?>
            </td>
            <td><a href="<?php echo U('Share/rewardOfModule',array('id'=>$list['id'],'token'=>$list['token']));?>" class="btn">修改</a></td>
