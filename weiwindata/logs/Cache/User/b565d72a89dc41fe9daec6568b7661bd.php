...
<?php if(is_array($class)): $i = 0; $__LIST__ = $class;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$c): $mod = ($i % 2 );++$i;?><tr>
	...
	<td>
  <?php if(is_array($lparr)): $i = 0; $__LIST__ = $lparr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$lp): $mod = ($i % 2 );++$i; if($lp['id'] == $c['lpid']): echo ($lp['LouPanTitle']); endif; endforeach; endif; else: echo "" ;endif; ?>
</td>

	<td class="norightborder"><?php if($c['sub'] == NULL): ?><a href="###" onclick="returnHomepage(<?php echo ($c["id"]); ?>,'<?php echo ($c["name"]); ?>')">选中</a><?php else: ?><a href="###" onclick="returnHomepage(<?php echo ($c["id"]); ?>,'<?php echo ($c["name"]); ?>')">选中</a> &nbsp; <a href="?g=User&m=Img&a=editClass&id=<?php echo ($c["id"]); ?>">详情</a><?php endif; ?></td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
</table>
