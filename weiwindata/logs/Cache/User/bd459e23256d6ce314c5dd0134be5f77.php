          ...
          <input name="delall" type="hidden" value="del">
           <input name="wxid" type="hidden" value="gh_423dwjkewad">
            <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
              ...
              <tbody>
              <?php if($list == null): ?><tr>
                  <td colspan="5" style="text-align:center; height:30px;"><strong>暂无达人信息</strong></td>
                </tr><?php endif; ?>
    			   <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                  <td><?php echo ($list["sharename"]); ?></td>
                  <td><?php echo ($list["customname"]); ?></td>
                  <td><?php echo ($list["customphone"]); ?></td>
				          <td><?php echo ($list["LouPanTitle"]); ?></td>
                  <td class="norightborder">
    						<a href="<?php echo U('Custom/edit',array('id'=>$list['id'],'token'=>$list['token']));?>">修改</a>&nbsp;&nbsp;
                <a href="<?php echo U('Custom/del',array('id'=>$list['id'],'token'=>$list['token']));?>"  onclick="return(confirm('确定要删除吗？'))">删除</a>
    				   </td>
