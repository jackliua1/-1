...
<div class="cLineB">
  <h4>修改楼盘奖励</h4><a href="<?php echo U('Wechat_behavior/rewardinfo');?>" class="right btnGrayS vm" style="margin-top:-27px">返回</a>
 </div> 
	...
	<div class="control-group">
		...
		<div class="controls">
			<select name="loupanid" style="height:30px;">
				<?php if(is_array($lp)): $i = 0; $__LIST__ = $lp;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($vo['id'] == $list['loupanid']): ?>selected<?php endif; ?>><?php echo ($vo["LouPanTitle"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
			</select>
			<span class="maroon">*</span><span class="help-inline"></span>
		</div>
	</div>
	<div class="control-group">
		<label for="Tel" class="control-label">销售状态</label>
		<div class="controls">
			<select name="statusid" style="height:30px;">
				<?php if(is_array($sta)): $i = 0; $__LIST__ = $sta;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($vo['id'] == $list['statusid']): ?>selected<?php endif; ?>><?php echo ($vo["salesstatus"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
			</select>
			<span class="maroon">*</span><span class="help-inline"></span>
		</div>
	</div>
	<div class="control-group">
		<label for="Tel" class="control-label">奖励类型</label>
		<div class="controls">
			<select name="rewardid" style="height:30px;">
				<?php if(is_array($rew)): $i = 0; $__LIST__ = $rew;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($vo['id'] == $list['rewardid']): ?>selected<?php endif; ?>><?php echo ($vo["rewardname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
			</select>
			<span class="maroon">*</span><span class="help-inline"></span>
		</div>
	</div>
   <div class="form-actions">
			<button id="bsubmit" type="submit" data-loading-text="提交中..." class="btnGreen">保存</button>　<a href="<?php echo U('Wechat_behavior/rewardinfo');?>" class="btnGray vm">取消</a>
		</div>
</form>
  </div> 
