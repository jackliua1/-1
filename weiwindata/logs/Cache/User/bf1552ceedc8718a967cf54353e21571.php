          ...
          <input name="delall" type="hidden" value="del">
           <input name="wxid" type="hidden" value="gh_423dwjkewad">
			...
			<th>保护日期（天）</th>
              ...
              <tbody>
              <?php if($groups == null): ?><tr>
                  <td colspan="7" style="text-align:center; height:30px;"><strong>暂无项目信息</strong></td>
                </tr><?php endif; ?>
    			   <?php if(is_array($groups)): $i = 0; $__LIST__ = $groups;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                  <td width="30px"><?php echo ($list["lp_id"]); ?></td>
                  <td width="50px"><?php echo ($list["pro_name"]); ?></td>
                  <td><?php echo ($list["cit_name"]); ?></td>
                  <td><?php echo ($list["are_name"]); ?></td>
                  <td><?php echo ($list["LouPanTitle"]); ?></td>
				  <td><?php echo ($list["djname"]); ?></td>
				  <td><?php echo ($list["djtel"]); ?></td>
				          <!-- <td><?php echo (mb_substr(htmlspecialchars_decode($list["projectinfo"]),0,20,'utf-8')); ?>...</td> -->
                  <td><?php echo ($list["houseSort"]); ?></td>
			<td><?php echo (date("Y-m-d",$list["BeginTime"])); ?></td>
                    <td><?php echo ($list["houseSortse"]); ?></td>

                  <td class="norightborder">

                

    						<a href="<?php echo U('Wechat_group/groupSet',array('id'=>$list['lp_id'],'type'=>'1'));?>">修改</a>&nbsp;&nbsp;
                <a href="<?php echo U('Wechat_group/groupDelete',array('id'=>$list['lp_id']));?>"  onclick="return(confirm('确定要删除吗？'))">删除</a>
    				   </td>
