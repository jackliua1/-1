...
<div class="cLineB">
  <h4>经纪人信息维护</h4><a href="<?php echo U('Share/index',array('token'=>$token));?>" class="right btnGrayS vm">返回</a>
 </div> 
  ...
  <form action="" method="post" class="form-horizontal form-validate" novalidate="novalidate">
	...
	<div class="control-group">
		...
		<div class="controls">
			<input type="text" name="Name" id="Name" maxlength="10" class="span4" value="<?php echo ($list["Name"]); ?>" data-rule-required="true"><span class="maroon">*</span><span class="help-inline"></span>
		</div>
	</div>
	<div class="control-group">
		<label for="Tel" class="control-label">经纪人电话：</label>
		<div class="controls">
	...
	<div class="control-group">
		...
		<div class="controls">
			<input type="password" name="password" id="password" class="span4" value=""><span class="maroon">*</span><span class="help-inline">请填写登录密码</span>
		</div>
	</div>
	<div class="control-group">
		<label for="userName" class="control-label">所属公司：</label>
		<div class="controls">
				<select name="brokerage_id" data-placeholder="选择所属公司" style="height:30px;width:200px;" id="type" class="chosen-select">
					<option value=""></option>
					<?php if(is_array($type)): $i = 0; $__LIST__ = $type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($vo[id] == $list[brokerage_id]): ?>selected<?php endif; ?>><?php echo ($vo["usersname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
		</div>
	</div>

	<!-- <div class="control-group">
		<label for="YlMoney" class="control-label">已领取佣金：</label>
		<div class="controls">
			<input type="text" name="YlMoney" id="YlMoney" class="span4" data-rule-required="true" value="<?php echo ($list["YlMoney"]); ?>"><span class="maroon">*</span><span class="help-inline"></span>
		</div>
	</div>
	<div class="control-group">
		<label for="DueMoney" class="control-label">未领取佣金：</label>
		<div class="controls">
			<input type="text" name="DueMoney" id="DueMoney" class="span4" data-rule-required="true" value="<?php echo ($list["DueMoney"]); ?>"><span class="maroon">*</span><span class="help-inline"></span>
		</div>
	</div> -->
	<div class="control-group">
		<label for="Mtime" class="control-label">创建日期：</label>
		<div class="controls"><?php echo (date('Y-m-d',$list["Mtime"])); ?>
			<input type="hidden" name="Mtime" id="Mtime" class="span4" data-rule-required="true" value="<?php echo (date('Y-m-d',$list["Mtime"])); ?>" readonly>
		</div>
	</div>
   <div class="form-actions">
			<button id="bsubmit" type="submit" data-loading-text="提交中..." class="btnGreen">保存</button>　<a href="<?php echo U('Share/index');?>" class="btnGray vm">取消</a>
		</div>
</form>
  </div> 
  <script>
