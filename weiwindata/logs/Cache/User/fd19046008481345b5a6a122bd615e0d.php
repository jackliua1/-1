...
<div class="cLineB">
  <h4>积分明细</h4><a href="javascript:history.back(-1);" class="right btnGrayS vm" style="margin-top:-27px">返回</a>
 </div> 
  <div class="msgWrap bgfc">
  <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
        <thead>
          <tr>
            <th class="span1">分享人</th>
            <th class="span1">分享项目</th>
            <th class="span3">浏览量</th>
            <th class="span3">获得积分</th>
          </tr>
        </thead>
        <tbody>
          <?php if(!$list): ?><tr>
            <td colspan="4" style="text-align:center; height:30px;"><strong>暂无积分记录</strong></td>
            </tr>
          <?php else: ?>
          <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
            <td><?php echo ($list['Name']); ?></td>
            <td>
              <?php if($list['type'] == 1): echo ($list['LouPanTitle']); else: ?>购买商品<?php endif; ?>
            </td>
            <td><?php echo ($list['views']); ?></td>
            <td><?php if($list['type'] == 1): ?>+<?php else: ?>-<?php endif; echo ($list['hascredit']); ?></td>
