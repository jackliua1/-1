...
<div class="cLineB">
  <h4>新增经纪人</h4><a href="<?php echo U('Share/index',array('token'=>$token));?>" class="right btnGrayS vm">返回</a>
 </div> 
  <div class="msgWrap bgfc">
  <form action="<?php echo U('Share/addAgentHandle',array('token'=>$token));?>" onsubmit="return checkinfo();" method="post" class="form-horizontal form-validate" novalidate="novalidate">
	<div class="control-group">
		<label for="title" class="control-label">经纪人姓名：</label>
		<div class="controls">
	...
	<div class="control-group">
		...
		<div class="controls">
			<input type="password" name="password" id="password" class="span4" data-rule-required="true" value="<?php echo ($list["password"]); ?>"><span class="maroon">*</span><span class="help-inline">请填写登录密码</span>
		</div>
	</div>
	<div class="control-group">
		...
		<div class="controls">
				<select name="brokerage_id" data-placeholder="选择所属公司" id="brokerage_id" style="" class="chosen-select">
					<option value=""></option>
				<?php if(is_array($type)): $i = 0; $__LIST__ = $type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['id']); ?>"><?php echo ($vo['usersname']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
		</div>
	</div>
