...
<!--返回上一页-->
    ...
    <form action="#" method="post">
        <div class="reg_content">
			<div class="khgl_dl">
				<?php if(is_array($lplists)): $i = 0; $__LIST__ = $lplists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><span><?php echo ($vo["LouPanTitle"]); ?></span><?php endforeach; endif; else: echo "" ;endif; ?>
			</div>
            <input type="text"  value="<?php echo ($acinfo['Name']); ?>" disabled class="reg_cone" name="Name" id="name">
            <input type="text"  value="<?php echo ($acinfo['Tel']); ?>" disabled onkeyup="this.value=this.value.replace(/[^\d]/g,'')" name="Tel" id="tel"> 

            <!-- <input type="text" value="<?php echo ($acinfo['LouPanTitle']); ?>" id="lp" onclick="showboxa('lpinfo')"  class="but_selectlist"> -->
        ...
        <input type="hidden" name="Uid" id="Uid" value="<?php echo ($acinfo['Uid']); ?>">
        <input type="hidden" name="id" value="<?php echo ($acinfo['ID']); ?>">
    </form>
...
<script language="javascript">
	...
	$("#logout").on('click',function(){
		location.href="<?php echo U('SaleManager/register',array('token'=>$token));?>";
	});
</script>
</html>