...
<?php if(is_array($products)): $i = 0; $__LIST__ = $products;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p): $mod = ($i % 2 );++$i; if(empty($p['detail']) != true): if(is_array($p['detail'])): $i = 0; $__LIST__ = $p['detail'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><li>
				<span class="pic" style="margin-left:0px;"><img src="<?php echo ($p["logourl"]); ?>" width="75" height="75"></span>
				...
				<p><label>数量：</label><?php echo ($row["count"]); ?>　<label>小计：</label><span class="price" style="width:80%">￥<?php echo ($row["price"]); ?></span></p>
				</span>
	...
	<?php else: ?>
		<li>
			...
			<span class="con" style="margin-left:0px;  dispaly:block; width:100%">
			<i class="t"><?php echo ($p["name"]); ?></i>
			<p><label>数量：</label><?php echo ($p["count"]); ?>　<label>小计：</label><span class="price" style="width:80%">￥<?php echo ($p["price"]); ?></span></p>
			</span>
...
<p>支付方式： 货到付款</p>
<?php elseif($cartData['paymode'] == 4): ?>
<p>支付方式： 积分支付</p>
<?php elseif($cartData['paymode'] == 5): ?>
<p>支付方式： 佣金支付</p>
<?php else: ?>
